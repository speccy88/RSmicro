...
class Historian:
 def __init__(self,path,queue_size=4096,batch_size=128): self.path=Path(path); self.queue=asyncio.Queue(queue_size); self.batch_size=batch_size; self.high_water=0; self.dropped=0; self.write_count=0; self.errors=0; self._conn=None; self._task=None
 def open(self):
